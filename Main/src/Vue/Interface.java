...
//ajouté pour connecter le popup de suppression 
import Controler.SupprimerControler;
import Vue.PopupSupprimer;
// ajouté pour le popup de modifierdocument
import Controler.ModifierDocumentControler;
...
import Controler.ModifierAdherentControler;
...
private Tab creerPageDocuments() {
    ...
    private TableView<Document> creerTabVueDocuments() {
        ...
        //SupprimerControler supprimerControler = new SupprimerControler(bibliothequeManager, table);
        
        // Controleur de modification de document 
        ModifierDocumentControler modifierDocumentControler = new ModifierDocumentControler(bibliothequeManager, table);
        ...
        // controleur de suppression 
        SupprimerControler supprimerControler = new SupprimerControler(bibliothequeManager, table,statusCol);
            ...
            // Action de supprimer : déléguer au controleur
            supprimerBtn.setOnAction(e -> {
                Document doc = getTableView().getItems().get(getIndex());
                supprimerControler.confirmerEtSupprimer(doc);
            });
            // Action de Modifier : déléguer au controleur 
            modifierBtn.setOnAction(e -> {
                Document doc = getTableView().getItems().get(getIndex());
                modifierDocumentControler.ouvrirPopupModification(doc);
            });
        }
...
private Tab createMembersTab() {
    ...
    private TableView<Adherent> createMembersTable() {
        ...
        actionsCol.setCellFactory(col -> new TableCell<>() {
            ...
            //==========Controler qui connecte le bouton à la fenêtre de modification 
            modifierBtn.setOnAction(e -> {
            Adherent adherent = getTableView().getItems().get(getIndex());
            new Controler.ModifierAdherentControler(bibliothequeManager, adherent, getTableView());
        });
        }
            @Override
            protected void updateItem(Void item, boolean empty) {
...
//RetourController retourController = new RetourController(bibliothequeManager);
    ...
    private TableView<Emprunt> createBorrowingsTable() {
        ...
        //Nouvelle colonne Alerte 
        TableColumn<Emprunt, String> alertCol = new TableColumn<>("Alerte");
        alertCol.setPrefWidth(150);

        // Que doit contenir la cellule (texte brut)
        alertCol.setCellValueFactory(cellData -> {
        Emprunt e = cellData.getValue();
        if (e == null) return new SimpleStringProperty("");

        LocalDate aujourdHui = LocalDate.now();

        // Emprunt en cours, pas encore rendu, et en retard
        if (e.getStatut_Emprunt()
                && e.getDate_RetourReelle() == null
                && e.getDate_RetourPrevue() != null
                && e.getDate_RetourPrevue().isBefore(aujourdHui)) {
            return new SimpleStringProperty("RETARD : " + ChronoUnit.DAYS.between(e.getDate_RetourPrevue(), LocalDate.now())+ " JOURS ");
        }

        return new SimpleStringProperty("");
    });

    // Style (rouge + gras) si "RETARD"
    alertCol.setCellFactory(col -> new TableCell<Emprunt, String>() {
        @Override
        ...
        table.getColumns().addAll(Arrays.asList(idCol, docCol,docCol2, memberCol,memberCol2, borrowDateCol, dueDateCol,realDateCol,alertCol));
